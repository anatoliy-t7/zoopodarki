<div class="fixed bottom-0 left-0 z-50 w-full max-w-sm p-4 space-y-3 toasts-container sm:p-6"
  ...
  <template x-for="toast in toasts.filter((a)=>a)" :key="toast.index">
    <div @mouseenter="cancelRemovalWithNewer(toast.index)" @mouseleave="scheduleRemovalWithOlder(toast.index)"
      @click="remove(toast.index)" x-show="toast.show===1"
      x-transition:enter="transform ease-out duration-500 transition"
      x-transition:enter-start="translate-y-2 opacity-0 sm:translate-y-0 sm:translate-y-10"
      x-transition:enter-end="translate-y-0 opacity-100 sm:translate-y-0"
