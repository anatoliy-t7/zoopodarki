...
<div class="py-6 space-y-4">
      ...
      <div class="relative block w-full max-w-2xl">
        <div class="absolute -top-8" style="left: calc({{ $greenLine }}% - 1.7rem)">
          <svg class="w-7 h-7" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <path class="text-green-500 stroke-current" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" stroke-width="1.5" d="M2 2h1.74c1.08 0 1.93.93 1.84 2l-.83 9.96a2.796 2.796 0 0 0 2.79 3.03h10.65c1.44 0 2.7-1.18 2.81-2.61l.54-7.5c.12-1.66-1.14-3.01-2.81-3.01H5.82M16.25 22a1.25 1.25 0 1 0 0-2.5 1.25 1.25 0 0 0 0 2.5zm-8 0a1.25 1.25 0 1 0 0-2.5 1.25 1.25 0 0 0 0 2.5zM9 8h12" />
          </svg>
        </div>
        <div class="w-full bg-white rounded-full">
          <div class="h-2 text-xs leading-none text-center text-white transition-all bg-green-500 rounded-full" style="width: {{ $greenLine }}%">
          </div>
        </div>
  ...
  <div class="justify-between block md:flex md:space-x-6">
    <div class="block w-full px-6 pt-4 pb-6 space-y-8 bg-white md:w-8/12 rounded-2xl">
      ...
      <div x-cloak x-data class="py-4 text-sm text-center text-gray-600">
        Пожалуйста,
        авторизуйтесь или зарегистрируйтесь для продолжения оформления заказа.
      </div> --}}
      <div x-cloak x-data class="py-4 text-xl text-center text-red-600">
      ...
      <div>
        ...
        <div class="items-center justify-start block pt-2 space-y-6 md:space-y-0 md:space-x-6 md:flex">
          @if ($contactSelected)
          <div class="items-start justify-start block w-full px-4 py-3 space-y-4 md:w-9/12 md:space-y-0 md:space-x-4 md:flex md:h-14">
      ...
      <div x-data="{ toggle: {{ $orderType }} }">
        ...
        <div class="space-y-4">
          <div class="items-center justify-start block space-y-4 md:space-y-0 md:space-x-4 md:flex">
            <div class="inline-flex w-full leading-none text-gray-400 bg-gray-200 border-2 border-gray-200 rounded-2xl md:w-6/12 h-14">

              <button wire:click="getOrderType(0)" x-on:click="toggle = 0" :class="{ 'bg-white text-green-600': toggle === 0 }" class="inline-flex items-center w-6/12 px-4 py-2 transition-colors duration-300 ease-in rounded-2xl focus:outline-none hover:text-green-600">
                <svg :class="{ 'text-green-600': toggle === 0 }" class="w-8 h-6 pr-2 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              ...
              <button wire:click="getOrderType(1)" x-on:click="toggle = 1" :class="{ 'bg-white text-green-600': toggle === 1 }" class="inline-flex items-center w-6/12 px-4 py-2 transition-colors duration-300 ease-in rounded-2xl focus:outline-none hover:text-green-600">
                <svg :class="{ 'text-green-600': toggle === 1 }" class="w-8 h-8 pr-2 fill-current" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" viewBox="0 0 24 24">
            ...
            <div class="flex items-center w-full md:w-6/12">
              <svg class="hidden w-8 h-8 pr-2 text-gray-300 fill-current md:block" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          ...
          <div class="w-full">
            ...
            <div :class="{ 'flex': toggle == 0, 'hidden': toggle == 1}" class="flex flex-col items-center justify-start gap-6 md:flex-row">
              ...
              <div x-cloak>
                ...
                <div class="items-center justify-start block gap-6 md:flex">
                  ...
                  <div class="w-full md:w-3/12">
                    <x-modal :width="'6xl'">
                      <x-slot name="button">
                        <div x-on:click="$dispatch('init-map')" class="flex items-center justify-center w-full gap-1 px-4 bg-gray-100 border border-gray-300 cursor-pointer hover:border-gray-400 h-14 rounded-xl">
          ...
          <div x-cloak :class="{ 'block': toggle == 0, 'hidden': toggle == 1}" class="relative w-full pt-6 space-y-6 fadeIn ">
                ...
                <div wire:ignore id="date" class="flex items-center justify-center w-full overflow-hidden splide">
                  ...
                  <div class="w-full max-w-xs md:max-w-md splide__track">
                    <ul class="splide__list">
                      ...
                      <li class="relative cursor-pointer splide__slide hover:bg-white" :class=" date == '{{ $item['date'] }}' ? 'text-green-600 bg-white' : 'text-gray-500 bg-gray-50'">
                        <div x-on:click="$wire.set('date', '{{ $item['date'] }}')" class="block px-2 py-3 text-center " :class=" date == '{{ $item['date'] }}' ? 'border-2 border-green-500' : 'border-r-2 border-y-2 border-white'">
                          <div class="pb-1 font-semibold">
                            {{ $item['number'] }}
                          </div>
                          <div class="text-xs capitalize truncate" :class=" date == '{{ $item['date'] }}' ? 'text-green-600 ' : 'text-gray-500 '">
                            {{ $item['name'] }}
                          </div>
                        </div>
                      </li>
      ...
      <div x-data="{payment: 'online'}" class="pt-2">
        ...
        <div class="content-center justify-start block space-y-4 md:flex md:space-y-0 md:space-x-4">
          ...
          <div class="inline-flex w-full leading-none text-gray-400 bg-gray-200 border-2 border-gray-200 md:h-14 rounded-2xl md:w-6/12">
            <button wire:click="paymentType(0)" x-on:click="payment = 'online'" :class="{ 'bg-white text-green-600': payment == 'online' }" class="inline-flex items-center w-6/12 px-4 py-2 transition-colors duration-300 ease-in rounded-2xl focus:outline-none hover:text-green-600">

              <x-tabler-credit-card :class="{ 'text-green-600': payment == 'online' }" class="inline mr-2 stroke-current w-7 h-7" />

              <span>Оплата онлайн</span>
            </button>
            <button wire:click="paymentType(1)" x-on:click="payment = 'cash'" :class="{ 'bg-white text-green-600': payment == 'cash' }" class="flex items-center justify-center w-6/12 px-4 py-2 space-x-4 transition-colors duration-300 ease-in rounded-2xl focus:outline-none hover:text-green-600">
              <svg :class="{ 'text-green-600': payment == 'cash' }" class="w-8 h-8 fill-current" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" viewBox="0 0 24 24">
                <g fill="none">
                  <path d="M17 9V7a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2m2 4h10a2 2 0 0 0 2-2v-6a2 2 0 0 0-2-2H9a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2zm7-5a2 2 0 1 1-4 0a2 2 0 0 1 4 0z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                </g>
              </svg>
          ...
          <div class="flex items-center w-full md:w-6/12">
            ...
            <div class="text-gray-400">
              ...
              <div x-cloak :class="{ 'flex': payment == 'cash', 'hidden': payment == 'online'}" class="items-center justify-end space-x-3">
                <span class="block text-sm text-right text-gray-700 ">Нужна сдача с</span>
                <div class="w-32">
                  <input wire:model="needChange" type="number" class="placeholder-gray-300 field" placeholder="500" inputmode="numeric">
                </div>
    ...
    <div class="block w-full space-y-2 md:w-4/12">
          ...
          <div class="py-2">
            <div class="flex items-center justify-between space-x-2 ">
              <div class="p-2 bg-white ">
                ...
                <a class="w-full" target="_blank" href="{{ route('site.product', ['catalogslug' => $item->associatedModel['catalog_slug'], 'categoryslug' => $item->associatedModel['category_slug'], 'productslug' => $item->associatedModel['product_slug']]) }}">
                  <img loading="lazy" class="object-fill w-12 h-full" src="{{ $item->associatedModel['image'] }}" alt="{{ $item->name }}">
                </a>
                @endif
              </div>
              ...
              <div class="w-full">
                ...
                <a class="block w-full hover:underline" target="_blank" href="{{ route('site.product', ['catalogslug' => $item->associatedModel['catalog_slug'], 'categoryslug' => $item->associatedModel['category_slug'], 'productslug' => $item->associatedModel['product_slug']]) }}">
                  {{ $item->name }}
                </a>
            ...
            <div class="py-2">
              <div class="flex items-center justify-between space-x-2 ">
                <div class="p-2 ">
                  ...
                  <a class="w-full" target="_blank" href="{{ route('site.product', ['catalogslug' => $shelterItem->associatedModel['catalog_slug'], 'categoryslug' => $shelterItem->associatedModel['category_slug'], 'productslug' => $shelterItem->associatedModel['product_slug']]) }}">
                    <img loading="lazy" class="object-fill w-12 h-full" src="{{ $shelterItem->associatedModel['image'] }}" alt="{{ $shelterItem->name }}">
                  </a>
                  @endif
                </div>
                ...
                <div class="w-full">
                  ...
                  <a class="block w-full hover:underline" target="_blank" href="{{ route('site.product', ['catalogslug' => $shelterItem->associatedModel['catalog_slug'], 'categoryslug' => $shelterItem->associatedModel['category_slug'], 'productslug' => $shelterItem->associatedModel['product_slug']]) }}">
                    {{ $shelterItem->name }}
                  </a>
      ...
      <div class="px-4 py-2">
        <div class="py-4 space-y-4 text-gray-700 border-t border-b border-gray-200">
          ...
          <div class="flex space-x-2 text-sm">
            ...
            <span class="flex justify-end w-full font-bold md:w-8/6">{{ $needChange }}<span class="pl-1">₽</span></span>
          </div>
          @endif
          @else
          <div class="flex space-x-2 text-sm">
      ...
      <div class="sticky p-6 top-5 ">
        <div class="block px-6 md:px-0">
          {{--
          <button wire:click="createOrder" class="relative w-full px-3 py-4 text-lg font-bold text-white uppercase bg-green-500 hover:bg-green-600 rounded-2xl disabled:opacity-50 disabled:cursor-not-allowed" wire:loading.attr="disabled">
            <span wire:loading wire:target="createOrder" class="absolute top-0 flex items-center h-full left-6">
              <svg class="w-6 h-6 mr-3 -ml-1 text-white animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          ...
          <div class="pt-4 text-xs text-gray-500">
            Нажимая кнопку "Оформить заказ", Вы соглашаетесь c <a class="leading-tight text-green-500" href="/page/privacy-policy" target="_blank">условиями
