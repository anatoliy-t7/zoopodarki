...
if (document.getElementById("mobmenu")) {
  ...
  window.addEventListener("scroll", function () {
    ...
    if (currentScroll > _lastScroll && !body.classList.contains("scroll-down")) {
      // down
      body.classList.remove("scroll-up");
      body.classList.add("scroll-down");
    } else if (currentScroll < _lastScroll && body.classList.contains("scroll-down")) {
      // up
      body.classList.remove("scroll-down");
      body.classList.add("scroll-up");
    }
...
observer.observe(); // after filter products call it again

window.livewire.on("lozad", function () {
  observer.observe();
}); // phone mask
