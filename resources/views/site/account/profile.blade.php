...
<div class="flex items-center">
  <div class="md:w-8/12 md:mx-auto">
    <form method="POST" action="{{ route('account.user.update', $user->id) }}">
      ...
      <div class="pt-2">
        <label class="block">
          <span class="block pb-1 font-bold text-gray-700">Адрес</span>
          <input class="block w-full mt-1 field" name="address" value="{{ old('address', $user->address) }}">
          @error ('address')
          <span class="text-xs text-red-600">
            {{ $message }}
          </span>
      ...
      @if ($user->discount)
      <div class="pt-4">
        У вас есть дисконтная карта <b>5%</b>
      </div>
      @endif
