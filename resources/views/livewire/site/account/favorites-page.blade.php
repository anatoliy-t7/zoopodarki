<div class="space-y-2">
  ...
  <div class="flex justify-start px-4 py-1 text-xs font-semibold text-gray-400 xl:px-0">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center justify-between">
        <a class="p-1 hover:underline" href="{{ route('account.favorites') }}">
        ...
        <div class="grid grid-cols-1 gap-8 p-6 bg-white rounded-2xl md:grid-cols-3 sm:grid-cols-2 ">
          @foreach ($favorites as $favorite)
            <div wire:key="{{ $loop->index }}"
              ...
              <a href="{{ route('site.product', ['catalogslug' => $favorite->product->categories[0]->catalog->slug, 'categoryslug' => $favorite->product->categories[0]->slug, 'productslug' => $favorite->product->slug]) }}"
                class="p-2">
