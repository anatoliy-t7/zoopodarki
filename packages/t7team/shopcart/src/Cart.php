...
use T7team\Shopcart\Exceptions\InvalidConditionException;
use T7team\Shopcart\Exceptions\InvalidItemException;
use T7team\Shopcart\Exceptions\UnknownModelException;
use T7team\Shopcart\Helpers\Helpers;
use T7team\Shopcart\Validators\CartItemValidator;
...
class Cart
{
    ...
    public function addItemCondition($productId, $itemCondition)
    {
        if ($product = $this->get($productId)) {
            $conditionInstance = '\\T7team\\Shopcart\\CartCondition';

            if ($itemCondition instanceof $conditionInstance) {
                // we need to copy first to a temporary variable to hold the conditions
                // to avoid hitting this error "Indirect modification of overloaded element of T7team\Shopcart\ItemCollection has no effect"
    ...
    public function condition($condition)
    {
        ...
        if (! $condition instanceof CartCondition) {
            throw new InvalidConditionException('Argument 1 must be an instance of \'T7team\Shopcart\CartCondition\'');
        }
     ...
     * @return bool
     */
    public function removeItemCondition($itemId, $conditionType)
    {
        ...
        if ($this->itemHasConditions($item)) {
            ...
            // we will iterate through all of it and check if the type matches
            // to the given type the user wants to remove, if so, remove it
            ...
            // an instance of a Condition, if so, we will check if the type matches
            // on the given condition type the user wants to remove, if so,
