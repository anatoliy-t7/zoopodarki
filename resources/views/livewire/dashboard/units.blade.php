...
<div>
  ...
  <div x-data="handler" @get-items.window="getItems(event)" @new.window="openForm(event)" @save.window="saveForm(event)"
    ...
    <div class="flex items-center justify-between w-full pb-2 space-x-6">
      ...
      <h3 class="text-2xl">Единицы измерения</h3>

      @can('create')
        <button @click="openForm()" id="add" title="Создать новую единицу измерения"
      ...
      <x-dashboard.table>
        ...
        <x-slot name="body">
          @forelse($units as $key => $unit)
            <x-dashboard.table.row wire:key="{{ $unit->id }}">
              ...
              <x-dashboard.table.cell class="flex items-center justify-end invisible group-hover:visible">
                ...
                <button @click="openForm" wire:click="openForm({{ $unit->id }})" title="Edit"
    ...
    <div>
      ...
      <x-dashboard.modal>
        ...
        <div class="block">
          ...
          <div class="space-y-1">
            <div class="font-bold">Название единицы измерения</div>
            <input wire:model.defer="editUnit.name" type="text" class="max-w-sm">
            @error('name')
              <span class="text-sm text-red-500">Поле обязательно для заполнения</span>
            @enderror
          </div>

          <div class="pt-6">
            @can('edit')
              <button wire:click="save" class="text-white bg-green-500 btn" wire:loading.attr="disabled">
                Сохранить
              </button>
            @endcan
          </div>
