...
<div>
  ...
  <div x-data="handler" @get-items.window="getItems(event)" @new.window="openForm(event)" @save.window="saveForm(event)"
    ...
    <div class="flex items-center justify-between w-full pb-2 space-x-6">
      ...
      <h3 class="text-2xl">Роли пользователей</h3>
        ...
        <x-slot name="body">
          ...
          <x-dashboard.table.row>
            ...
            <x-dashboard.table.cell>
              ...
              <div class="p-2">{{ $permission->name }}</div>
              @endforeach
            </x-dashboard.table.cell>

            <x-dashboard.table.cell class="flex items-center justify-end invisible group-hover:visible">

              <button @click="openForm" wire:click="openForm({{ $role->id }})" title="Edit"
    ...
    <div>
      ...
      <x-dashboard.modal class="overflow-y-auto">
        ...
        <div class="space-y-4">
          ...
          <div class="py-2 ">
            <div class="pb-1">Права для роли</div>
            <div class="h-full max-h-screen p-4 space-y-3 overflow-y-auto bg-white rounded-lg">
              @foreach($permissions as $permission)
              <div wire:key="{{ $permission->index }}" class="container-checkbox">
                <span class="text-xs">{{ $permission->name }}</span>
                <input wire:model.lazy="rolePermissions" value="{{ $permission->name }}" type="checkbox">
